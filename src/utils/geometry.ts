...
// Build offset endpoints for a segment
function buildOffsets(a: Point, b: Point, half: number) {
    const dir = normalize(sub(b, a));
    const n = perp(dir);
    return {
        dir,
        n,
        left: { a: add(a, mul(n, half)), b: add(b, mul(n, half)) },
        right: { a: add(a, mul(n, -half)), b: add(b, mul(n, -half)) },
    };
}
...
function computeIntersection(
    pivot: Point,
    dirA: Point, offsetA: number,
    dirB: Point, offsetB: number
): Point | null {
    // Normal vectors
